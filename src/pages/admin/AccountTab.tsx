...
export function AccountTab({
  ...
  const updateOrgName = useUpdateOrganizationName();
  ...
  const saveCompanyName = async () => {
    ...
    try {
      await updateOrgName.mutateAsync(trimmed);
      toast.success(t("accountTab.toast.companyNameSaved"));
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t("accountTab.toast.couldNotSaveCompanyName"));
    } finally {
  ...
  // Plan-limit / grace-period state — surfaced in the Billing section since
  // it's a billing-downgrade consequence, not a location-browsing concern
  // (location browsing/management itself lives in the Concepts tab).
  ...
  const show = (s: "account" | "users" | "billing") => !section || section === s;
    ...
    <div className="space-y-4">
      ...
      {show("account") && (
        <section className="card-surface p-4 space-y-3">
          <p className="section-label">{t("accountTab.companyDetails")}</p>
          <label className="space-y-1 block">
            <span className="text-xs text-muted-foreground font-medium">{t("accountTab.companyName")}</span>
            <input
              type="text"
              value={companyName}
          ...
          <button
            ...
            disabled={companyNameSaving || !companyName.trim() || companyName.trim() === org?.name}
            className={cn(
              "py-2.5 px-4 rounded-xl text-sm font-semibold transition-colors",
              companyNameSaving || !companyName.trim() || companyName.trim() === org?.name
      ...
      {/* Owner details: My Account + Security — side by side */}
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <div className="space-y-1">
              ...
              {currentAccount?.id && currentAccount.is_owner && (
      ...
      {/* Billing */}
      {show("billing") && <section className="space-y-4">
        <div>
        ...
        {isLocationOverLimit && (
          <div className="card-surface border border-status-warn/30 bg-status-warn/5 px-4 py-3 space-y-3">
            <div className="space-y-1">
                ...
                {locations.map((location) => (
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2 shrink-0">
                      {activeLocationSet.has(location.id) ? (
                        <span className="text-xs text-sage font-medium">{isGraceExpired ? t("accountTab.active") : t("accountTab.graceWindow")}</span>
                      ) : inactiveLocationSet.has(location.id) ? (
                        <span className="text-xs text-muted-foreground font-medium">{t("accountTab.readOnly")}</span>
                      ) : null}
                      <input
                        type="checkbox"
