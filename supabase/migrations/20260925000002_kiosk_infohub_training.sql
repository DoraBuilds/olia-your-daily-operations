-- Kiosk "Infohub" (#903): get_kiosk_library now returns Training content as
-- well as Library, tagging every folder/document with its `section`. Access
-- rules are unchanged (same as 20260915000003): Owner sees everything,
-- otherwise org-wide items or ones shared with this member / role / location,
-- and a document also needs its folder to be visible. Archived docs excluded.
