...
- Never merge or push changes directly to `main`.
- All code changes must go through a feature branch and a pull request.
- Treat `main` as protected, even if local tooling would allow direct commits or merges.
- If work is prepared for review, the correct outcome is a PR branch, not a direct merge to `main`.
- Before doing any optional git workflow steps, prefer non-destructive commands and preserve any unrelated user changes in the worktree.
