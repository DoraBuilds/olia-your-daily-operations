...
if ! command -v supabase >/dev/null 2>&1; then
  echo "Supabase CLI is required for Docker-backed integration tests." >&2
  exit 1
fi
...
if ! command -v bun >/dev/null 2>&1; then
  if [ -x "$HOME/.bun/bin/bun" ]; then
    ...
    echo "bun is required to run integration tests." >&2
    exit 1
  fi
else
  BUN_BIN="bun"
...
if [ -z "$api_url" ] || [ -z "$anon_key" ] || [ -z "$service_role_key" ]; then
  echo "Could not resolve local Supabase env from \`supabase status -o env\`." >&2
  exit 1
fi
...
"$BUN_BIN" x vitest run --config vitest.integration.config.ts "$@"
